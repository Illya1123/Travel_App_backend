...
export const createMomoPayment = async (req, res) => {
    try {
        ...
        const response = await axios.post(process.env.MOMO_ENDPOINT, {
            ...
            extraData: "",
            signature: crypto.createHmac("sha256", process.env.MOMO_SECRET_KEY)
                .update(`accessKey=${process.env.MOMO_ACCESS_KEY}&amount=${totalPrice}&extraData=&ipnUrl=${process.env.MOMO_IPN_URL}&orderId=${orderId}&orderInfo=Thanh toán đặt tour&partnerCode=${partnerCode}&redirectUrl=${process.env.MOMO_REDIRECT_URL}&requestId=${requestId}&requestType=captureWallet`)
                .digest("hex"),
        });
...
export const momoCallback = async (req, res) => {
    try {
        ...
        if (resultCode === 0) {
            ...
            return res.status(400).json({ success: false, message: message || "Thanh toán thất bại" });
        }
    } catch (error) {
...
export const checkMomoTransaction = async (req, res) => {
    try {
        ...
        if (resultCode === 0) {
            ...
            return res.status(200).json({ success: true, message: "Thanh toán thành công", data: response.data });
        } else {
            return res.status(400).json({ success: false, message: "Giao dịch chưa hoàn tất hoặc thất bại", data: response.data });
        }
    } catch (error) {
